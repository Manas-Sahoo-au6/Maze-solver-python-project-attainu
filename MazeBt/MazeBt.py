      #***********************************************************************************#
      # date:-9/06/2020 :PYTHONE PROJECT (MAZE SOLVER USING BACK-TRACKING) by MANAs:      #
      #***********************************************************************************#
...
def maze_runner(row, col):
    if maze[LENGTH-1][LENGTH-1] == 0:  # condition checking whether the destination element is 0 or not
        return False
    if (row == LENGTH-1) and (col == LENGTH-1):  # if the final destination reached then
        solution[row][col] = 1
        return True
    if (row >= 0 and col >= 0 and row < LENGTH  and col < LENGTH and solution[row][col] == 0 and maze[row][col] == 1):
...
def print_function(sol):
    ...
    file2.write("***\n\n" + "the  path followed is " + printDirec(sol) + "reach to the destination: \n\n")
    for i in sol:
        for j in i:
            file2.write(" " + str(j) + " ")
        file2.write('\n')
    ...
    file2.write("***\n" + "Time of execution at : " + time.strftime("%d-%m-%Y * %H:%M:%S\n" + "***\n\n"))
...
#********************************************************************************************************#
#              FUNCTION THAT CHECK THE MOVMENT OF CELL TO REACH THE MAZE TO IT'S DESTINATION  
...
def printDirec( solvedMaze ):    #call of direction printing function
    ...
    visited = [[False for _ in range(n)] for _ in range(n)]   #make a variable to store in a array cell intial as false 
    arr = ""           #take blank string variable that store the the direction of movement 
    i = j = 0
...
if __name__ == "__main__":
  ...
  #TAKING FILE ARGUMENTS FROM COMMAND LINE.  (STEP-1)
    ...
    LENGTH = len(maze)
    solution = [[0]*LENGTH for _ in range(LENGTH)]    #making a solution matrix and set all cell elemnt to zero initial  
    ...
    if(maze_runner(0,0)):  #(STEP-3)
        ...
        file2.write("***\n-1\n")
        file2.write(" sorry there is no any path available to reach destination.")
        now = datetime.datetime.now()
        file2.write("\nTime of execution at : " + now.strftime("%d-%m-%Y * %H:%M:%S") + "\n***\n\n\n")
